...
Modified version of GCN, GAT, and GraphSAGE UPFD implementations from:
...
parameters = {'politifact gcn profile': (0.01, 50),
              'politifact gcn content': (0.001, 150),
              'politifact gcn spacy': (0.001, 50),
              'politifact gcn bert': (0.001, 50),
              'politifact gat profile': (0.01, 50),
              'politifact gat content': (0.001, 150),
              'politifact gat spacy': (0.01, 50),
              'politifact gat bert': (0.01, 50),
              'politifact sage profile': (0.01, 50),
              'politifact sage content': (0.001, 150),
              'politifact sage spacy': (0.01, 50),
              'politifact sage bert': (0.001, 50),
              'gossipcop gcn profile': (0.001, 50),
              'gossipcop gcn content': (0.001, 50),
              'gossipcop gcn spacy': (0.001, 50),
              'gossipcop gcn bert': (0.001, 50),
              'gossipcop gat profile': (0.001, 50),
              'gossipcop gat content': (0.001, 50),
              'gossipcop gat spacy': (0.001, 50),
              'gossipcop gat bert': (0.001, 50),
              'gossipcop sage profile': (0.001, 50),
              'gossipcop sage content': (0.001, 50),
              'gossipcop sage spacy': (0.001, 50),
              'gossipcop sage bert': (0.001, 50),
              }
...
for ds in datasets:
    for mdl in models:
        for ftr in features:
            ...
            lr, epochs = parameters[f'{ds} {mdl} {ftr}']
            ...
            for seed in seeds:
                ...
                step = 0
                while step <= num_splits or Model.base:
                    ...
                    parser.add_argument('--model', type=str, default=mdl, help='model type, [gcn, gat, sage]')
                    ...
                    if not Model.base and num_splits != 0:
                    ...
                    embeddings = Model.embeddings[total_passes*(es_epoch-1):total_passes*(es_epoch)]
                    pred_probs = Model.pred_probs[total_passes*(es_epoch-1):total_passes*(es_epoch)]
                    ...
                    test_emb = np.array(np.concatenate(Model.embeddings[-test_passes:]))
                    test_pred_probs = np.concatenate(Model.pred_probs[-test_passes:])
                    ...
                    if Model.base:
                        ...
                        if num_splits != 0:
                            exclude = run_helper.get_exclude(ds, train_emb, train_pred, train_graph_labels, num_exclude, title=f'{ds} upfd {mdl} {ftr} {es_epoch} epochs')
                            if num_splits == -1:
                        ...
                        if save_for_slices:
                            ...
                            settings = {'dataset':ds, 'model':f'upfd {mdl}', 'feature':ftr, 'seed':seed, 'epochs':es_epoch}
                            np.save('last_run/settings', settings)
                            
                        Model.base = False
                        
                    # Plot pca of test embeddings to see what it looks like
                    _ = run_helper.get_exclude(ds, test_emb, test_pred, test_graph_labels, 0, title=f'{ds} upfd {mdl} {ftr} test {es_epoch} epochs', save_graph=False)
                    ...
                    plt.title(f'{args.dataset} upfd {args.model} {args.feature}')
            ...
            with open('performance_log.txt' ,'a') as outfile:
                ...
                outfile.write(f'{ds} upfd {mdl} {ftr}\n')
