...
Modified version of GNNCL implementation from:
...
# Early stopping doesn't seem to work well for this model
# Changing the learning rate also doesn't seem to do anything?
parameters = {'politifact profile': (0.001, 60),
    ...
    for ftr in features:
        ...
        for seed in seeds:
            ...
            while step <= num_splits or Net.base:
                ...
                train_loader_2 = DenseDataLoader(training_set, batch_size=args.batch_size, shuffle=False)
                ...
                for epoch in tqdm(range(args.epochs)):
                    ...
                    [_, _, _, _, _, _, _], _ = test(train_loader_2)
                ...
                if Net.base:
                    ...
                    if num_splits != 0:
                        exclude = run_helper.get_exclude(ds, train_emb, train_pred, train_graph_labels, num_exclude, title=f'{ds} gnncl {ftr} {es_epoch} epochs')
                        if num_splits == -1:
                    ...
                    if save_for_slices:
                        ...
                        settings = {'dataset':ds, 'model':'gnncl', 'feature':ftr, 'seed':seed, 'epochs':es_epoch}
                        np.save('last_run/settings', settings)
                        
                    Net.base = False
                    
                # Plot pca of test embeddings to see what it looks like
                _ = run_helper.get_exclude(ds, test_emb, test_pred, test_graph_labels, 0, title=f'{ds} gnncl {ftr} test', save_graph=False)
                ...
                plt.title(f'{args.dataset} gnncl {args.feature}')
