...
Modified version of GCNFN implementation from:
...
parameters = {'politifact profile': (0.001, 50),
              ...
              'politifact spacy': (0.001, 50),
              'politifact bert': (0.001, 50),
        ...
        for seed in seeds:
            ...
            while step <= num_splits or Net.base:
                ...
                if Net.base:
                    ...
                    if num_splits != 0:
                        exclude = run_helper.get_exclude(ds, train_emb, train_pred, train_graph_labels, num_exclude, title=f'{ds} gcnfn {ftr} {es_epoch} epochs')
                        if num_splits == -1:
                    ...
                    if save_for_slices:
                        ...
                        settings = {'dataset':ds, 'model':'gcnfn', 'feature':ftr, 'seed':seed, 'epochs':es_epoch}
                        np.save('last_run/settings', settings)
                        
                    Net.base = False
                    
                # Plot pca of test embeddings to see what it looks like
                _ = run_helper.get_exclude(ds, test_emb, test_pred, test_graph_labels, 0, title=f'{ds} gcnfn {ftr} test', save_graph=False)
                ...
                plt.title(f'{args.dataset} gcnfn {args.feature}')
